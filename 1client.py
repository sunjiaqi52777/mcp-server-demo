...
class MCPClient:
    def __init__(self):
        ...
        # 所有资源都通过 self.exit_stack 统一管理，确保用完后能自动关闭和清理。
        self.exit_stack = AsyncExitStack()
        self.client = OpenAI()

    async def connect_to_server(self):
        # 创建了一个 StdioServerParameters 实例
        # 指定用 python 命令启动 server.py 文件（即服务器端）
        server_params = StdioServerParameters(
            command='python',
            args=['server.py'],
            env=None
        )
        # stdio_transport 是一个元组，包含 stdio（读端）和 write（写端），用于和服务器通信。
        stdio_transport = await self.exit_stack.enter_async_context(
            stdio_client(server_params))
        stdio, write = stdio_transport

        # 用 stdio 和 write 创建了一个 ClientSession，并同样用 exit_stack 管理其生命周期。
        # self.session 就是后续和服务器交互的会话对象。
